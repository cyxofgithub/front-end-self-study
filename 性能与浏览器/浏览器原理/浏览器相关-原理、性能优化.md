...
（3）哪些资源用强缓存、哪些用协商缓存及原因

实际工程中，按「资源 URL 是否会随内容变化」来分配缓存策略：

-   **强缓存（长缓存）**：带 hash 指纹的静态资源（JS、CSS、图片、字体等），如 `app.abc123.js`、`style.def456.css`。
    -   原因：文件名里的 hash 由内容生成——内容一变，hash 就变，URL 也变，浏览器拿到的是全新 URL，天然不会命中旧缓存；内容不变时 URL 不变，可以放心长缓存。
    -   做法：`Cache-Control: max-age=31536000, immutable`（缓存一年，后续无需再向服务器验证）。

-   **协商缓存（每次验证）**：不带 hash 的资源，尤其是入口 HTML（`index.html`）。
    -   原因：HTML 的 URL 固定（如 `/` 或 `/index.html`），但内容会随构建更新（引用的 JS/CSS 文件名变了）。若对 HTML 用强缓存，用户会一直拿到旧 HTML，从而加载不到新的 JS/CSS，导致页面不更新。
    -   做法：`Cache-Control: no-cache`，每次向服务器验证（ETag / Last-Modified），有更新返回新内容，否则 304 复用本地缓存。

-   **一句话总结**：HTML 走协商缓存，保证「及时拿到最新的资源引用」；带 hash 的静态资源走强缓存，保证「一次下载、长期复用」；两者配合实现「更新及时性」与「性能」的平衡。
