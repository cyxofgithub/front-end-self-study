...
| 场景 | 建议 | 原因 |
| --- | --- | --- |
| 业务协议高度定制，主要是浏览器与单个服务端实时交互 | WebSocket | 链路直接，消息格式和状态机可以完全自定义 |
| 多设备、多订阅者，需要按主题分发 | MQTT | Broker 和 topic 能解耦生产者与消费者 |
| 需要 QoS、遗嘱、保留消息、持久会话 | MQTT | 协议已有标准能力，避免重复建设 |
| 浏览器要接入已有 MQTT 体系 | MQTT over WebSocket | 兼顾浏览器网络限制与 MQTT 消息语义 |
| 高频位置等“新值覆盖旧值”的遥测 | MQTT QoS 0 | 减少确认开销，旧数据也没有重传价值 |
| 告警、控制指令等不能轻易丢的消息 | MQTT QoS 1 + 业务幂等 | 至少投递一次，并防止重复执行 |
