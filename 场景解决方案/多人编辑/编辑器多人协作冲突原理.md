## 编辑器多人协作冲突的核心解决方案（图解版）

> 一句话先记住：OT 是“改操作”，CRDT 是“改数据结构”。

---

### OT（Operational Transformation）

#### 结论

OT 适合中心化实时协作场景：客户端本地先执行提升手感，服务端负责顺序化，客户端对远程操作做 transform 保证最终一致。

#### 1) OT 总体架构图

```mermaid
flowchart LR
    userA[UserA]
    userB[UserB]
    clientA[ClientA]
    clientB[ClientB]
    pendingA[PendingQueueA]
    pendingB[PendingQueueB]
    server[OTServer]
    mainDoc[MainDoc]

    userA --> clientA
    userB --> clientB
    clientA -->|本地apply| pendingA
    clientB -->|本地apply| pendingB
    clientA -->|发送localOp| server
    clientB -->|发送localOp| server
    server -->|顺序化应用| mainDoc
    server -->|广播remoteOp| clientA
    server -->|广播remoteOp| clientB
    clientA -->|transform(remoteOp,pending)| pendingA
    clientB -->|transform(remoteOp,pending)| pendingB
```

关键点：

-   本地先 `apply`，用户输入无明显延迟。
-   服务端串行化操作，减少“全网乱序”复杂度。
-   客户端维护 `pending`（已本地应用、未确认的本地操作）。
-   接收远程操作后先 transform，再 apply。

#### 2) OT 并发冲突时序图（A/B 同时在 0 插入）

```mermaid
sequenceDiagram
    participant A as ClientA
    participant S as Server
    participant B as ClientB

    A->>A: 本地apply opA=insert(0,"a")
    B->>B: 本地apply opB=insert(0,"b")

    A->>S: send opA
    B->>S: send opB

    S->>S: apply opA
    S->>S: opB1=transform(opB,opA)
    S->>S: apply opB1

    S-->>A: broadcast opB1
    S-->>B: broadcast opA

    A->>A: transform(opB1,pendingA)后apply
    B->>B: transform(opA,pendingB)后apply
```

一致性目标：

```text
apply(apply(S,opA), transform(opB,opA))
=
apply(apply(S,opB), transform(opA,opB))
= S_final
```

#### 3) OT 远程操作处理流水图

```mermaid
flowchart TB
    recv[收到remoteOp r]
    t1["[r1,l1p]=transform(r,l1)"]
    t2["[r2,l2p]=transform(r1,l2)"]
    t3["[r3,l3p]=transform(r2,l3)"]
    apply[apply r3到本地文档]
    update["pending=[l1p,l2p,l3p]"]

    recv --> t1 --> t2 --> t3 --> apply --> update
```

速记：

-   远程操作 `r` 需要依次穿过本地 `pending` 队列。
-   transform 是“双向更新”：远程位移 + 本地队列重写。
-   处理完再应用远程，并更新本地待确认队列。

---

### CRDT（Conflict-free Replicated Data Type）

#### 结论

CRDT 的核心是让并发操作天然可合并：不依赖中心化 transform，下发顺序不同也能收敛。

#### 1) CRDT 文本数据模型图

```mermaid
flowchart LR
    charNode[CharNode]
    idField[id]
    valueField[value]
    leftField[left]
    rightField[right]
    deletedField[deleted]
    orderRule[全序规则]
    viewLayer[渲染层]

    charNode --> idField
    charNode --> valueField
    charNode --> leftField
    charNode --> rightField
    charNode --> deletedField
    idField --> orderRule
    leftField --> orderRule
    rightField --> orderRule
    orderRule --> viewLayer
    deletedField --> viewLayer
```

关键点：

-   位置由 `left/right + id` 表达，不是数组下标。
-   并发插入冲突通过全序规则（如 `id` 字典序）裁决。
-   删除通常是 tombstone（`deleted=true`），避免按下标误删。

#### 2) CRDT 增量同步时序图（生产版：经 Server 交换）

```mermaid
sequenceDiagram
    participant A as ClientA
    participant S as SyncServer
    participant B as ClientB

    A->>A: 本地生成并apply opA6
    B->>B: 本地生成并apply opB4 opB5

    A-->>S: 上报stateVector {A:6,B:3}
    B-->>S: 上报stateVector {A:5,B:5}
    S->>S: 计算双方缺失集
    S-->>A: 下发缺失 B4..B5
    S-->>B: 下发缺失 A6

    Note over B,S: B 网络抖动短暂断线
    S->>S: 暂存B待补洞区间

    B->>S: reconnect + lastSeenVector
    S-->>B: replay missing ops

    A->>A: 幂等apply缺失操作
    B->>B: 幂等apply缺失操作
```

速记：

-   生产中通常是 `Client <-> SyncServer` 交换，客户端不一定互联直连。
-   版本向量负责“去重 + 补洞”，不是冲突裁决器。
-   冲突裁决由 CRDT 数据结构内建规则处理。
-   重连后按缺失集回放，重复投递也安全（幂等）。

#### 3) 教学化小例子（并发同序号）

```text
A: H(num0,idA1), e(num1,idA2)
B: H(num0,idA1), l(num1,idB1)
```

合并后同 `num` 时按 `id` 全序排序；删除时按元素身份删除：

```json
{ "op": "delete", "targetId": "idB1" }
```

---

### OT vs CRDT 选型决策图

```mermaid
flowchart TD
    start[开始选型]
    offline{是否强依赖离线可编辑}
    dataComplex{是否有富文本或复杂JSON}
    central{是否接受中心化强依赖}
    scale{是否需要大规模多端并发}
    chooseCRDT[优先CRDT]
    chooseOT[优先OT]
    mixed[混合方案]

    start --> offline
    offline -->|是| chooseCRDT
    offline -->|否| dataComplex
    dataComplex -->|是| chooseCRDT
    dataComplex -->|否| central
    central -->|是| scale
    central -->|否| chooseCRDT
    scale -->|小到中等| chooseOT
    scale -->|大规模| mixed
```

---

### 对照表（保留决策信息）

| 维度     | OT                    | CRDT                        |
| -------- | --------------------- | --------------------------- |
| 冲突处理 | 运行时 transform 操作 | 数据结构天然可合并          |
| 架构依赖 | 常见为中心化服务端    | 去中心化或中心化都可        |
| 离线能力 | 可做但工程复杂        | 原生友好                    |
| 数据类型 | 纯文本最常见          | 文本/富文本/JSON/集合       |
| 成本特征 | transform 规则复杂    | 元数据与 tombstone 管理复杂 |

---

### 实践建议（前端）

1. 纯文本 + 中心化在线编辑器：优先 OT。
2. 离线优先 + 多端同步 + 复杂结构：优先 CRDT（如 Yjs / Automerge）。
3. 追求稳定性时，先做“最小协作闭环”：本地 apply、远程同步、重连补洞、幂等处理，再扩展高级能力（撤销栈、权限、富文本语义）。

---

### 异常场景补充：全播时某个节点网络异常

#### 场景结论

服务端全播时，单个节点网络异常不会破坏全局收敛；关键在于“正常节点继续前进，异常节点重连后补齐缺失操作”。

#### 全播异常时序图（单节点超时）

```mermaid
sequenceDiagram
    participant S as Server
    participant C1 as Client1
    participant C2 as Client2异常
    participant C3 as Client3

    S-->>C1: broadcast opN
    S-->>C2: broadcast opN
    S-->>C3: broadcast opN

    C1-->>S: ack opN
    C3-->>S: ack opN
    C2-xS: timeout/断线 未ack

    Note over S,C1: 正常节点继续处理 opN+1, opN+2
    S->>S: 记录C2缺失区间

    C2->>S: reconnect + lastSeen
    S-->>C2: replay missing ops
    C2->>C2: 幂等apply并追平版本
```

#### OT 下怎么处理

-   服务端可按连接维护每个客户端的确认进度（如最后确认的序号）。
-   对异常节点采用“重连后补发缺失操作”，客户端收到后仍按 `transform(remote,pending)` 处理。
-   正常在线节点不回滚、不等待异常节点，避免整体阻塞。
-   若补发失败，可退化为“快照 + 增量”恢复。

#### CRDT 下怎么处理

-   异常节点重连后先交换 `state vector/version vector`。
-   双方计算缺失集后补洞，缺失操作可乱序到达，依赖幂等和可交换性收敛。
-   即使重复投递也安全（去重 + 幂等 apply）。
-   若本地积压过大，同样可用“快照 + 缺失增量”加速追平。
