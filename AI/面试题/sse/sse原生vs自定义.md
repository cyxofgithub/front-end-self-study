...
### 为什么要自定义 SSE（相比原生 EventSource 的优势）

一句话结论：原生 SSE 只提供了最基础的推送能力，自定义可以在**协议层**和**客户端实现层**双向扩展。
...
> 补充：上表是**协议层**的扩展优势；本文下面的 fetch/XHR 实现属于**客户端实现层**自定义，额外获得的能力是：支持 POST 请求、自定义请求头（如鉴权 token）、`AbortController` 主动中断连接——这些都是原生 `EventSource`（仅 GET、无法设置请求头）做不到的。

### 原生写法：EventSource

```javascript
// 1. 建立连接（仅支持 GET，无法自定义请求头）
const eventSource = new EventSource('/api/sse');

// 2. 接收默认消息（服务端未指定 event 字段时触发）
eventSource.onmessage = (event) => {
    console.log('收到消息:', event.data);
};

// 3. 监听自定义事件（对应服务端的 event: xxx 字段）
eventSource.addEventListener('progress', (event) => {
    console.log('进度:', event.data);
});

// 4. 错误处理：断开后浏览器会按 retry 间隔自动重连，
//    并自动携带 Last-Event-ID 请求头实现断点续传，无需手动处理
eventSource.onerror = (error) => {
    console.error('连接异常:', error);
};

// 5. 主动关闭（close 后不再自动重连）
...
```javascript
res.writeHead(200, {
    'Content-Type': 'text/event-stream', // SSE 核心响应头
    'Cache-Control': 'no-cache',
    Connection: 'keep-alive',
});
res.write('data: hello\n\n'); // 默认消息 → 触发 onmessage
res.write('event: progress\ndata: 50%\n\n'); // 自定义事件 → 触发 addEventListener('progress')
```

> 服务端详细原理见 [底层原理](../../../网络与安全/计算机网络/sse/底层原理.md)。
