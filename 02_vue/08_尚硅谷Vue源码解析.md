...
## 09_diff 处理新旧两个节点是同一个节点时

![image-20220302223601295](08_尚硅谷Vue源码解析.assets/image-20220302223601295.png)
...
一部分是新旧节点不一样的情况就直接进行暴力增删

一部分是同一节点的情况，如果和之前的节点完全一致就不用做任何操作；如果有 text 属性就看下 text 属性一不一样，不一样就替换，如果没有 text 说明就是有 childrennode 了，这是最复杂的情况，需要做精细化比较，也是diff的核心，其实最终目的的就是删除多余的节点，添加新增的节点，移动节点位置等操作。
...
- 新前与旧后（将这个节点移动到最前面）
- 新后与旧前（将节点移动到最后面）
- 如果这四种都没查找到，就建个map去暴力查找，确定这个节点是要增删还是要移动还是要删除还是改动
