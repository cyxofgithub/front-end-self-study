...
每一种灯光都能被设置为关闭/开启：
```javascript
light.setEnabled(false); // 关闭
light.setEnabled(true);  // 开启
```
...
## 指定照亮哪些物体
当创建一个灯光时，场景中所有的物体都会被照亮。

有2种方法可以让某些物体不被照亮：一种是把需要排除的物体加入到excludedMeshes数组中，另一种是把需要照亮的物体加入到includedOnlyMeshes数组中，不在数组中的物体将不被照亮。

两个方法应都有各自适应的使用情况，可能物体的数量也是一个考量因素，例如：照亮的多于不照亮的物体，使用排除excludedMeshes，否则使用includedOnlyMeshes。在以下的示例中，展示两个属性的使用方法，即从light0排除了2个物体，只用light1照亮同样2个物体，最终的效果就是light0照亮其余23个物体，而light1照亮2个物体。
...
在某些情况下，从纹理定义灯光效果更好，因为灯光的漫反射只有常规的一个基本颜色，效果不够炫酷。想象一下我们正在模拟灯光透过教堂玻璃反射到地面上的五颜六色效果，如下图。对于投影或者KTV气氛灯，也基本是这种很多颜色的灯光效果。
