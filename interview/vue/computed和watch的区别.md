...
注意：`computed` 主要用于基于响应式数据进行同步计算和结果缓存，适合处理会被频繁读取的派生数据。**不推荐在 `computed` 中直接做异步操作**，因为 `computed` 的返回值要求是同步的。如果有异步需求，应当用 `watch` 实现；如果确实要结合异步处理，可以在 `computed` 中返回一个用于追踪异步状态的 ref 或 Promise，但这样通常会带来不可控的副作用，官方并不建议这么做。
