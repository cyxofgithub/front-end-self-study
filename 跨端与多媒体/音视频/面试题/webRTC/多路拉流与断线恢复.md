...
> 🎮 **配套代码**：下面的示例就是前端实际要写的控制器逻辑，覆盖宫格预算、可见性治理、冻结/掐断重连与主/子码流切换。
...
无人车链路里的四个动作：收车端推流 → 转封装成 RTP（H.264 进 H.264 出，只换包装）→ 提供 WHEP 端点 → 一份进 N 份出。这里的 `MockSFU` 就是这个角色的示意实现。
...
**信令去哪了？**（高频追问）WebRTC 规范故意不定义信令协议——SDP 怎么送到对端，随你用什么通道。通话示例里常见自建 WebSocket；WHEP 把通道标准化成**一次 HTTP 往返**，上面代码里的 fetch 就是信令本身：
...
连 `onicecandidate` 都没有：信令本该还有 ICE candidate 交换，但连的是服务器**公网地址**——Answer 自带公网 candidate，客户端发起的连通性检查（出站 UDP）可穿 NAT，无需 STUN/TURN 也不用 trickle。本例里 `LoopbackSignal.exchange()` 干的就是这次 fetch 的活，接口同为「送 Offer、回 Answer」才能一行换接真 SRS。
...
**再追问：所以 `onicecandidate` 不是必要操作？** 对，且任何场景都不是——它只是「候选边收边通知」的搬运口，候选由 ICE agent 自动收集并**自动写进 `localDescription.sdp`**（non-trickle 等收完直接读 SDP 即可，无需监听）。真正必要的是「**对端得知你的候选**」：P2P 双方都在 NAT 后、互相够不到，才必须交换地址（所以通话示例里这个事件满天飞）；拉流对端是公网服务器，你主动出站就够得到它，自然一条候选都不用发。判据一句话：**够得到对端就不用预告地址，够不到才要交换**。
...
**直觉数字**：单路 1080p 软解吃 20%~50% 一个核心，4 路并发足以打满一颗。解码线程、主线程、合成线程共享同一个 CPU 资源池——所以卡的不只是视频，是整页。4×4 + 1080p 时就会明显吃 CPU（环回还叠加 16 路编码压力）。
...
> 浏览器没有公开“硬解/软解切换”的直接 API。前端能做的是把**活跃的 RTCPeerConnection 数量**压住，并按可见性和优先级决定谁拿主码流、谁只保活子码流。
...
type StreamSpec = {
  ...
  mainUrl: string;
  subUrl?: string;
  priority?: number; // 置顶 / 告警路更高
  pinned?: boolean;
};

type StreamSlot = StreamSpec & {
  visible: boolean;
  player: WatchdogPlayer | null;
};
...
class VideoWall {
  ...
  constructor(private container: HTMLElement, private maxActive = 9) {
    this.observer = new IntersectionObserver(this.onIntersect, {
      root: this.container,
      rootMargin: '120px', // 提前预拉，滚动时不闪黑
    });
    document.addEventListener('visibilitychange', this.onPageVisibilityChange);
  }

  register(spec: StreamSpec) {
    spec.el.dataset.streamId = spec.id;
    const slot: StreamSlot = { ...spec, visible: false, player: null };
    this.slots.set(spec.id, slot);
    this.observer.observe(spec.el);
    this.sync();
  }
  ...
  private sync() {
    const candidates = [...this.slots.values()]
      .filter((slot) => slot.visible || slot.pinned)
      .sort((a, b) => this.score(b) - this.score(a));

    const activeIds = new Set(candidates.slice(0, this.maxActive).map((slot) => slot.id));
  ...
  private score(slot: StreamSlot) {
    return (slot.pinned ? 1000 : 0) + (slot.visible ? 100 : 0) + (slot.priority ?? 0);
  }

  private start(slot: StreamSlot) {
    if (slot.player) return;
    const url = slot.pinned || (slot.priority ?? 0) > 0 ? slot.mainUrl : slot.subUrl ?? slot.mainUrl;
    slot.player = new WatchdogPlayer(slot.el, url);
    slot.player.start().catch(() => this.stop(slot));
  }
...
- 主屏、置顶路、告警路保活主码流；
- 其余可见路优先走子码流；
- 视口外直接 `destroy()`，不要 `video.pause()`；
- 超过预算的路只保留封面或占位，不让它继续吃解码器。

### 4.5 三端代码：帧率/分辨率到底在哪控制

> **一句话结论：** 帧率/分辨率是**编码端定死**的参数——上行能真正改，服务端挑档/转码间接改，下行只能选流（浏览器没有挑 simulcast 档的 API）。

**① 上行（车端，真正改）**——编码器参数，后面谁都改不了：

```bash
ffmpeg ... -framerate 30 -video_size 1920x1080 ... -f flv rtmp://srs/live/vehicle_01
ffmpeg ... -vf "scale=640:480" -r 15 ... -f flv rtmp://srs/live/vehicle_01_sub   # 子流 480p15
```

WebRTC 推流端用 `scaleResolutionDownBy`（分辨率）+ `maxFramerate`（帧率）：

```typescript
params.encodings = [
  { rid: 'h', maxBitrate: 2_000_000, scaleResolutionDownBy: 1, maxFramerate: 30 }, // 主档
  { rid: 'l', maxBitrate: 150_000,  scaleResolutionDownBy: 4, maxFramerate: 15 },  // 低档
];
await sender.setParameters(params);
```

**② 服务端（SRS/SFU）**——两种做法：**挑档**（simulcast 转发，不重编码，≈0 CPU）或**转码**（重编码出子流，很贵）。生产主流的挑档不是 URL，而是 **SFU 按每个订阅者的 RTCP 带宽反馈自动挑档**。

**③ 下行（浏览器，只能选）**——换子流 URL，就是 4.4 那行：

```typescript
const url = slot.pinned ? slot.mainUrl : slot.subUrl ?? slot.mainUrl; // 主码流 vs 子码流
```

放大某路切高清 = **重建换流**（拉 `mainUrl` 等首帧 → 关旧连接 → 换 `srcObject`）；服务端支持 simulcast 时则**同连接切档**——发信令让服务端 `setPreferredLayers`（切档 API 在服务端，前端没有）。

> 🎮 **配套 demo**：[demo/simulcast-substream-demo.html](./demo/simulcast-substream-demo.html)——真实 canvas + RTCPeerConnection 环回推拉流，切换档位看下行 getStats 的分辨率/帧率/解码开销实时变化。
