...
#### webpack构建流程
  ...
  读入口 -> parse（入口代码）生成ast -> travse ast 收集依赖的绝对路径 -> 递归代码 transformcode 组织 fileInfo{ dep， code} -> generate 输出文件
...
- 然后到了大三下学期，也就是这学期我去到了上海百度实习，在百度实习期间主要是维护一个舆情系统，然后在四月份的时候开始负责这个项目考试院版本的开发，然后为了将这个新版本接入旧系统采用了微前端技术，然后项目的整体搭建也是采用了 vue3 + ts 的组合。
- 这就是我的一个校园经历，您有什么问题想问我的吗？
...
- React 是单向数据流，所以比 vue 更适合大型应用，vue 双向数据流，当组件嵌套层级过深的可能会造成数据难以追溯的问题，甚至可能一些错误的编码造成幽灵循环
- React 有函数式组件，类式组件
