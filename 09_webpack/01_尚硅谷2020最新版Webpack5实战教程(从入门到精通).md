  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        loader: 'url-loader',
        options: {
          // 图片大小小于8kb，就会被base64处理，优点：减少请求数量（减轻服务器压力），缺点：图片体积会变大（文件请求速度更慢）
      ...
      */
      {
        /*
          ...
          注意：只检查自己写的源代码，第三方的库是不用检查的,因此忽略node_modules
          airbnb(一个流行的js风格，检查规则) --> 需要下载 eslint-config-airbnb-base eslint-plugin-import 才能使用
          设置检查规则：
...
- html 文件: 默认不能使用 HMR 功能（html 不用做 HMR 功能，因为只有一个 html 文件，不需要再优化）
  ...
  使用 HMR 会导致问题：html 文件不能热更新了（不会自动打包构建，不能自动编译刷新浏览器）
...
### 5.1.2 source-map（还不是很明白）

可参考官方文档：https://webpack.docschina.org/configuration/devtool/

source-map：一种提供**源代码到构建后代码的映射**的技术 （如果构建后代码出错了，通过映射可以追踪源代码错误）

补充：其中一些值适用于开发环境，一些适用于生产环境。对于开发环境，通常希望更快速的 source map，需要添加到 bundle 中以增加体积为代价，但是对于生产环境，则希望更精准的 source map，需要从 bundle 中分离并独立存在。
...
module: {
  rules: [
    ...
    },
    {
      ...
      // 如果不用oneOf所有文件都要被每一个loader过一遍
      // 注意：在oneOf里不能有两个配置处理同一种类型文件（因为babel-loader和eslint-loader都是处理js类型的文件所以把eslint-loader提取出去放外面，不然其中一个会失效）
      oneOf: [
        {
          test: /\.css$/,
          use: [...commonCssLoader]
...
```
{
  test: /\.js$/,
  exclude: /node_modules/,
  loader: 'babel-loader',// 转译js代码为浏览器可识别的代码（es5）
  options: {
    presets: [
      [
        '@babel/preset-env',
...
module.exports = {
  entry: {
    // jquery 代表最终打包生成的 [name] 是 jquery
    // ['jquery] 代表 要打包的库是jquery
    jquery: ['jquery']
  },
  output: {
    // 输出出口指定
...
plugins: [
  ...
  // 将某个文件打包输出到build目录下，并添加该资源到html模板文件中
...
- 自动分析多入口chunk中，有没有公共的文件。如果有会打包成单独一个chunk(比如两个模块中都引入了jquery会被打包成单独的文件)（大小要超过30kb才会被单独打包出来）

3.import 动态导入语法：

（还有其他文件也想单独打包成一个chunk时使用）
...
#### 5.2.2.4 lazy loading（懒加载/预加载）（重点）
